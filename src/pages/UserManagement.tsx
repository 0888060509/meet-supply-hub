...
const UserManagement = () => {
  ...
  const filterUsers = () => {
    let results = [...users];
    if (searchTerm) {
      results = results.filter(user => user.username.toLowerCase().includes(searchTerm.toLowerCase()) || user.email.toLowerCase().includes(searchTerm.toLowerCase()));
    }
    if (roleFilter !== "all") {
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-6">
            <div className="relative flex-grow">
              ...
              <Input className="pl-10" placeholder="Search by username or email" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
          ...
          {filteredUsers.length > 0 ? <>
              <div className="border rounded-md shadow-sm overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {currentUsers.map(user => <TableRow key={user.id} className={user.status === "inactive" ? "opacity-60" : ""}>
                        <TableCell className="font-medium cursor-pointer hover:underline" onClick={() => handleViewUserDetails(user)}>
                          {user.username}
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex space-x-2">
                            ...
                            <Button variant={user.status === "active" ? "destructive" : "outline"} size="sm" onClick={() => handleToggleUserStatus(user.id)}>
              ...
              <p className="text-muted-foreground mb-4">
                {searchTerm || roleFilter !== "all" || statusFilter !== "all" ? "No matching users found. Try adjusting your search or filters." : "No users found. Click '+ Add User' to create one."}
              </p>
              {!(searchTerm || roleFilter !== "all" || statusFilter !== "all") && <Button onClick={() => setIsAddUserModalOpen(true)}>
                  <Plus className="mr-2 h-4 w-4" /> Add User
                </Button>}
            </div>}

          <AddUserModal isOpen={isAddUserModalOpen} onClose={() => setIsAddUserModalOpen(false)} onAddUser={handleAddUser} existingUsers={users} />
          
          {selectedUser && <>
              <EditUserModal isOpen={isEditUserModalOpen} onClose={() => setIsEditUserModalOpen(false)} onUpdateUser={handleUpdateUser} user={selectedUser} existingUsers={users} />
